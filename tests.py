...
def run():
    ...
    # ---------------------------------------------------------------
    section("Zebra cashier page (root) + shared-database WAL mode")
    r = client.get("/")
    page = r.get_data(as_text=True)
    check("/ serves the Zebra handheld cashier", r.status_code == 200 and "wedge-input" in page)
    check("payment step is on the page", "payment-modal" in page and "payment-received" in page)
    check("saved-carts UI is on the page", "cart-name-modal" in page and "parked-chips" in page)
    check("no camera scanner anywhere (DataWedge only)", "html5-qrcode" not in page and "scanner.js" not in page)
    for label, get_conn in (("store.db", db.get_store_db), ("sales.db", db.get_sales_db)):
        conn = get_conn()
