...
function addLookedUpProduct(product) {
  if (product.is_weighed) {
...
/* One Enter press = one decision. Product barcodes are digits (EAN/UPC), but
   Utsav plans printed shelf labels like "WT-RICE-BASMATI" for weighed bins, so
   ANY space-free value is first tried as an exact barcode; only if that misses
   do we fall back: all-digit values open Quick Add with the barcode attached
   (a real unknown product), lettered values run the name search (could just as
   well be a typed product name). */
async function handleWedgeEnter(value) {
  ...
  if (res.ok) {
    clearWedge();
    addLookedUpProduct(await res.json());
