...
# Two processes (this app + the original POS) share the same SQLite files.
# WAL lets one writer and many readers coexist; the busy timeout makes a
# briefly-locked write wait instead of raising "database is locked".
...
def _connect(path):
    """Shared-database connection: WAL + busy timeout on every connection.
    WAL is persistent once set, but setting it per-connection is harmless and
    guarantees it even on a fresh file."""
